...
### Security
- **Fuzz image pins `atheris` 3.0.0 and holds Dependabot below 3.1** — `.clusterfuzzlite/requirements-atheris.txt` moves from 2.3.0 to 3.0.0 (still hash-pinned); atheris 3.1+ ships only cp312+ wheels while the OSS-Fuzz base image runs Python 3.11, so Dependabot's first bump (#171) failed the fuzzing job with "No matching distribution". An `ignore` on that Dependabot entry keeps it from proposing 3.1+ until the base image's Python catches up
- **Fuzz image's `atheris` install is now hash-pinned** — `.clusterfuzzlite/build.sh` installs it from `.clusterfuzzlite/requirements-atheris.txt` with `--require-hashes` instead of a bare `atheris==2.3.0`, closing the Scorecard Pinned-Dependencies finding on that script. Dependabot tracks it via a new `pip` entry for `/.clusterfuzzlite`
- **Coverage-guided fuzz testing (ClusterFuzzLite + Atheris)** — `fuzz/fuzz_url_validation.py` and `fuzz/fuzz_log_sanitize.py` fuzz the deterministic pieces of `app.security.url_validation`'s SSRF-blocking logic and `app.security.log_sanitize.sanitize_for_log()`'s CWE-117 defense. Runs on every PR touching `app/security/`, `fuzz/`, or `.clusterfuzzlite/` (2 min, `fuzzing-pr.yml`) and weekly (1 hour, `fuzzing-batch.yml`); see `fuzz/README.md`
- **Malformed stored `endpoint_url` could crash outbound push and startup preflight instead of failing closed** — found by the new fuzzer within its first run: `urlparse()` itself raises `ValueError: Invalid IPv6 URL` for certain malformed netloc strings (e.g. `http://[not-a-real-ipv6`). That propagated uncaught out of `receiver_host_allowed()`, and on the failure branch both real callers — `app/events/pusher.py`'s `_revalidate_endpoint()` (the DNS-rebinding re-check before every outbound push; also its `_safe_host()` log helper) and `app/startup.py`'s stored-streams preflight check (reads `endpoint_url` straight from the database) — re-ran the same unguarded `urlparse(url).hostname`. New `safe_hostname()` returns `""` on an unparseable URL and is used in all of them: such a push is now blocked ("Blocked outbound push") and such a stored stream is reported as an allowlist violation, instead of raising
